...
def list_history_for_alice():
    ensure_inited()
    out = []
    for req_id, req in STATE["requests"].items():
        status = _compute_status(req)
        # persist computed expiry (so UI consistent)
...
def create_request_from_redirect(req: StartFromRedirect) -> Dict:
    """
    Alice is the initiator. She selects Bob/Chen from contacts.
    Alice is NOT footing the bill; everyone pays their own share (including Alice).
    When she proceeds, Alice immediately pays her share into escrow.
    """
    ensure_inited()
    ...
    # Alice immediately pays her share into escrow
...
def inbox_for(user: User):
    """
    For Bob/Chen wallet app: show requests where they are included and not yet paid.
    """
    ensure_inited()
    out = []
...
def _pay_internal(request_id: str, payer: User) -> Dict:
    ...
    if payer not in req["participants"]:
        raise ValueError(f"{payer} is not part of this payment request.")
    ...
    info = escrow_create(payer_wallet, req["merchant_address"], float(p["share_xrp"]))
    ...
    # If all paid -> finish all escrows and callback marketplace
...
def _settle_and_callback(req: Dict):
    ...
    # Call marketplace return_url
    try:
        httpx.post(req["return_url"], json=payload, timeout=15.0)
    except Exception:
        # For MVP demo, don't crash the settlement if callback fails.
        pass
